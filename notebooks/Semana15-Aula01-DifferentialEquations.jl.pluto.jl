...
# ╔═╡ 40ba561e-9a31-4c19-85ce-d1edd12e5b23
# Implementação
using DifferentialEquations, DataFrames, StatsPlots 

# ╔═╡ 917ea40c-1bf9-11ec-1438-1360ce5cde98
md"""
##### UFSC/Blumenau
...
# ╔═╡ 2a6e7280-1b24-4593-b690-0b9ae13eb45c
md"""
# Solução Numérica de Equações Diferenciais

### Usando  `DifferentialEquations.jl` 

"""

# ╔═╡ 8b2a59ca-47ff-4c09-84a9-e25c32db24fb
md"""
## Exemplo 1
- Seja o PVI

$$\frac{dy}{dt} = f(y,p,t)\tag{PVI},$$
em que $p$ é o vetor dos parâmetros do sistema. Considerando $t\in[0,1]$.
"""

# ╔═╡ a52f9b06-a207-4e16-bc7d-fcfa37b7ce5c
let
	f(y,p,t) = 1.01*y
	y₀ = .5
	...
	plot(sol)
	plot!(t-> 0.5*exp(1.01*t))
end

# ╔═╡ c6f6edd9-fe1a-4941-ac5d-b4f8122327c7
md"""
## Exemplo 2: Resolvendo sistema de EDOs
...
# ╔═╡ 534ed935-a902-4cd1-9b9a-4f10e49aa05e
...
# ╔═╡ ae3ce027-f3fb-4cbf-9685-f64c840d952e
let
	u₀ = [1.0, 0.0, 0.0]
	p = [10, 28, 8/3] #parãmetros [σ, ρ, β]
	tspan = (0.0,100.0)
...
# ╔═╡ 47fa1f33-4723-4763-b8b2-b347af7107ea
md"""
## Exemplo 3: Pêndulo
...
# ╔═╡ e0a8525a-933f-476a-af5d-61ded6126363


# ╔═╡ 5ce85e47-e357-4e70-96c0-9afe69e8006d
function pendulo!(du, u, p, t)
	θ, ω = u
	l, m, g = p
	du[1] = ω
	du[2] = -3/2 * (g/l) * sin(θ) + 3/(m*l^2)*0.1*sin(t)
end

# ╔═╡ b8cdfdda-3aac-4388-b23b-d084144be036
let
	u₀ = [0.01,0.0]
	p = [1.0, 1.0, 9.81] #parâmetros [l, m, g]
	tspan = (0.0,10.0)
	prob = ODEProblem(pendulo!,u₀,tspan,p)
	sol = solve(prob, reltol = 1e-8, abstol = 1e-8)
	plot(sol)
end

# ╔═╡ 80296e6e-087e-4a04-a570-a0b574244b28
md"""
## Exemplo 4: Sistemas Lineares de EDOs

$$u' = Au, A\in \mathbb{R}^{n\times n}$$
"""

# ╔═╡ 2da9b040-8382-470a-89df-e710c34091ee
let
	A = [1.0  0 0 -5
	     4 -2 4 -3
	    -4  0 0  1 
...
# ╔═╡ 29ee5601-be1b-4223-9a10-e0fc6a40c994
md"""
### Modelo SIR
"""

# ╔═╡ b84dd627-8aa6-4c31-8635-8eaeb5dce671
md"""
* Modelo SIR é um PVI
...
# ╔═╡ dceb59ea-8866-4eb8-ac22-a267b7192963
function sir_edo!(du,u,p,t)
	(S, I, R) = u
	(β, γ) = p
	N = S + I + R
	du[1] = -β*S*I/N
...
# ╔═╡ 8d65669e-039e-4f04-87f6-7a6fcca75bec
begin
	n = 60 #Discretização
	tfinal = 40. #
	t0 = 0.
	tspan = (t0,tfinal)
	t = range(t0, stop =  tfinal, length = n)
end

# ╔═╡ 15d58170-621b-40e0-95c6-9fb8ff55b3e8
# condições iniciais
y0 = [990., 10., 0.0]

# ╔═╡ d3a28c4f-ec07-4abf-94ad-12d95f4aee8d
# parâmetros
p = [0.75,0.25]

# ╔═╡ a16851de-0e7d-442b-8d06-e0ca095c857a
r0 = p[1]/p[2]

# ╔═╡ f2a815d3-a198-47d8-a0da-4a2f2bbd7bc8
md"""
### Rodar o modelo
"""

# ╔═╡ a7347b31-0494-4ab0-8504-32e22675d543
prob_edo = ODEProblem(sir_edo!,y0,tspan,p)

# ╔═╡ b93d3fb5-78fd-49d8-81c3-e32e0f800a7e
md"""
#### Resolve EDO
"""

# ╔═╡ 61d9df3f-5326-4b3b-8855-eb9d98c9c25f
sol_edo = solve(prob_edo);

# ╔═╡ 0337a3fe-4680-469c-b2bf-306af407a90c
df_ode = DataFrame(sol_edo)

# ╔═╡ fb405dd0-6de4-41ce-b531-80f2091c61d3
begin
plot(sol_edo,
		label = ["S" "I" "R"],
		xlabel = "tempo",
...
# ╔═╡ daf0e7f7-d12c-46f7-a87d-b1a132737166
md"""
### Modelo compartimental SEIR

* Os Estados são: suscetíveis ($S$), expostos ($E$), infectados ($I$) e removidos ($R$).

$S\to E \to I \to R$

* Total da população: $N = S(t) + E(t) + I(t) + R(t)$

* O modelo pode ser dado por 
$\begin{aligned} \frac{d S}{d t} &=-\beta S {I} \\ \frac{d E}{d t} &=\beta S {I}-\sigma E \\ \frac{d I}{d t} &=\sigma E-\gamma I \\ \frac{d R}{d t} &=\gamma I \end{aligned}$


* As transições são governadas pelos parâmetros 
  - taxa de transmissão ou de contato efetivo: $\beta(t)$ 
  - taxa de infecção: $\sigma$
  - taxa de recuperação (e de morte): $\gamma$
...
# ╔═╡ 1bc764a9-2074-48f1-9520-a7c9b5484bb7
# Implementação
function seir_edo!(du, u, p, t)
	(S, E, I, R) = u
	(β, σ, γ) = p
	N = S + E + I + R
	du[1] = -β*S*I/N #dS/dt
	du[2] =  β*S*I/N - σ*E #dE/dt
	du[3] = σ*E - γ*I #dI/dt
	du[4] = γ*I #dR/dt
end

# ╔═╡ 40ad155a-7c07-43dc-ac04-de4ad6b5a419
begin
	S0 = 220e7
	E0 = 132.
	I0 = 33.
	...
	γ = 1/18
	p_seir = [β, σ, γ]
	prob_seir =  ODEProblem(seir_edo!,y0_seir,tspan_seir,p_seir)
end

# ╔═╡ c13dc3a6-28b4-4abc-8482-84d2dc6cb77e
sol_seir = solve(prob_seir) # Runge-Kutta 4 ordem + incrementos

# ╔═╡ bca83104-0f5d-4de9-8d53-ddd26eb8caff
plot(sol_seir, labels = ["S" "E" "I" "R"])

# ╔═╡ 26cfd04d-4b1a-416d-96fa-3158c82df66f
areaplot(sol_seir.t, sol_seir', labels = ["S" "E" "I" "R"])

# ╔═╡ 8a6fcfca-4717-43c3-b0de-e08ddd8e5d86
