...
begin
	using ForwardDiff, DataFrames,  LinearAlgebra, Plots, LazySets, PlutoUI
end
...
# ╔═╡ b6b5b104-a0b0-4e27-b715-68639c040a36
...
> Dada uma função contínua escalar em uma variável $f$, seja $g$  uma função escalar   tal que $\bar x$  satisfaz $f(\bar x) = 0$ se e somente se $g(\bar x) = \bar x$. Então 
> 1. Começe com um ponto inicial  $x_{0}$.
> 2. Para $k=0,1,2, \ldots$, faça
> $x_{k+1}=g\left(x_{k}\right)$
> até que  $x_{k+1}$ satisfaça algum critério de parada.

* O método de Newton é uma iteração de ponto fixo. 
"""

# ╔═╡ a00512a3-c84b-4d4e-a830-e0a3e4ea6714
md"""
> **Teorema.** (Ponto fixo)
> Sejam $g \in C[a, b]$ e $a \leq g(x) \leq b$  para todo $x \in[a, b]$. Então existe um ponto fixo  $x^{*}$ de $g$ no intervalo $[a, b]$.
> Se, além disso,  a derivada $g^{\prime}$ existe e se há uma constante  $\rho<1$ tal que a derivada satisfaça
>
>$\left|g^{\prime}(x)\right| \leq \rho \quad \forall x \in(a, b)$
>então o ponto fixo  $x^{*}$ é único no intervalo. 
"""

# ╔═╡ 4291000b-68da-42a8-8e89-62f23f75a135
md"""
### Solução de Sistemas Lineares em Julia 

> Um problema fundamental de álgebra linear: resolver
>$Ax = b.$

* Veremos em seguida  fatoração `LU`

* Mas antes vamos falar do operador barra invertida `\` do Julia
...
with_terminal() do
	A = [ 1 2; 
		 2 -1.]
	b = [3, 2]
	@show x = A \ b #Operador barra inverstida
	norm(A*x - b)
end

# ╔═╡ 96d1bfed-1022-4c85-b841-b1d9af48814a
...
# ╔═╡ 9e125861-b73a-42ba-87dc-92f7c78d9432
md"""
...
# Implementação
function newton_siseq(F,  #F(x) = 0
					  J, #Jacobiana de F
					 x⁰; #Chute inicial 
	                 ε :: Float64 = 1.0e-5, itmax :: Int = 100)
	df = DataFrame(k = Int[], xᵏ = Vector[], normFᵏ = Float64[])
	k = 0
	xᵏ = x⁰
	Fᵏ = F(xᵏ)
	normFᵏ = norm(Fᵏ)
	while k ≤ itmax
		...
		dᵏ = J(xᵏ) \ Fᵏ #Resolver o SL de Newton usando barra invertida.
		xᵏ -=  dᵏ # xᵏ - dᵏ
		Fᵏ = F(xᵏ)
		normFᵏ = norm(Fᵏ)
		k += 1
...
begin
	F0(x) = [x[1]^2 - exp(-x[1]*x[2]), 
		    x[1]*x[2] + sin(x[1])]
	J0(x)= [2*x[1] + x[2]*exp(-x[1]*x[2])      x[1]*exp(-x[1]*x[2]);
		   x[2] + cos(x[1])                              x[1] ]
	x⁰ = [2.,1]
	J0(x⁰)
end
...
df0, status1  = newton_siseq(F0, J0, x⁰, ε = 1e-12)

# ╔═╡ edb05618-8994-45cb-b502-88ff1db7480f
 newton_siseq(F0, J0, [-3,0], ε = 1e-12)

# ╔═╡ ac894f56-d6dc-478b-9105-6a024f2e850c
 newton_siseq(F0, J0, [-1,0], ε = 1e-12)

# ╔═╡ 468a59f3-4896-4cb5-a456-946aefe79caa
md"""
#### Exemplo: 

> Seja $f_{1}\left(x_{1}, x_{2}\right)=x_{1}^{2}-2 x_{1}-x_{2}+1$ e $f_{2}\left(x_{1}, x_{2}\right)=x_{1}^{2}+x_{2}^{2}-1$. A equação  $f_{1}\left(x_{1}, x_{2}\right)=0$ descreve uma parábola enquanto  $f_{2}\left(x_{1}, x_{2}\right)=0$ descreve um círculo. Queremos encontrar a intersecção das duas curvas de nível acima. 
"""

# ╔═╡ e382735d-79f1-47ba-a415-1fd9c2742d4a
F1(x) = [x[1]^2 - 2*x[1] - x[2] + 1
		 x[1]^2	 + x[2]^1 - 1]

# ╔═╡ 6d9f4e65-9671-4469-b0f5-9820d592d861
J1(x) = ForwardDiff.jacobian(F1,x)

# ╔═╡ ab34de68-da00-4964-b692-087ccbc51d71
 df1, =  newton_siseq(F1, J1, [-2.,1], ε = 1e-12)

# ╔═╡ d3552cbe-1afa-4234-a6b7-d4887465b664
begin
	plot(x->x^2-2x + 1,-.5,1.5,leg=false,aspect_ratio=:equal,framestyle=:origin)
	plot!(Ball2([0.,0.],1.),leg=false, alpha=0, lalpha=1)
	scatter!([Singleton(v) for v ∈ df1.xᵏ])
end
...
git-tree-sha1 = "5f19ded192aaf8d444c9b9f4e69ed3c411282640"
