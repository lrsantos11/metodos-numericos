...
function calcula_raizdecimal(n::Number, t::Int64 = 0)
	a, b, status = calcula_raiz(n)
	if status != :TemRaiz
		for digit in 1:t
			ran = collect(range(a, b, length=11))
			for index in 2:11
				if ran[index]^2 == n
					return ran[index], ran[index], :RaizDecimal
				elseif ran[index]^2 > n
					a, b, status =  ran[index-1], ran[index], :IntervaloRaiz
					break
				end
			end
		end
...
with_terminal() do
	...
	for index in 1:num_ele
		...
		if m^2 > n
			@show  vet_r[index-1], m, :IntervaloRaiz
			break
		end
	end
end
