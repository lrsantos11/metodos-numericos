...
begin
	using PlutoUI, ForwardDiff, DataFrames, StatsPlots,  LinearAlgebra, LazySets
	plotly(), 	theme(:sand)
...
- Uma matriz simétrica é definida positiva se, e somente se, seus autovalores forem positivos
...
>
>$\phi({x}+{d})=\phi({x})+\nabla \phi({x})^{T} {d}+\frac{1}{2} {d}^{T} \nabla^{2} \phi({x}) {d}+\mathcal{O}\left(\|{d}\|^{3}\right).$
...
$F(x) := \nabla\phi(x) = 0.$
>**Algoritmo. (Método de Newton para Minimização sem restrições).** Considere o problema de minimizar $\phi({x})$ em $\mathbb{R}^{n}$, e seja ${x}^{0}$ um chute inicial.
>
>**para** $k=0,1, \ldots$, até que convirja, **faça**
>
>$\begin{array}{l}
>\text {1. Resolva } \nabla^{2} \phi\left({x}^{k}\right) {d}^{k}=- \nabla \phi\left({x}^{k}\right) \text { para } {d}^{k} \\
>\text {2. Atualize } {x}^{k+1} = {x}^{k}+{d}^{k}
>\end{array}$
>**fim**
"""
...
# ╔═╡ da0d9330-706f-4227-896c-63e233afe69a
	x1⁰ = [8,.2]	

# ╔═╡ eba22047-fd2d-456c-bcb4-c5a0943b1aaa
∇f(x1⁰)

# ╔═╡ af6c0204-006d-41d3-aa6c-aa9d54f22437
∇²f(x1⁰)

# ╔═╡ ad150109-bb07-4d2e-bb31-085aacfdea73
eigvals(∇²f(x1⁰))

# ╔═╡ a0a77b16-3b65-4cb5-aea0-33bd568e5d0f
begin
	df1,  = newton_siseq(∇f, ∇²f, x1⁰,ε = 1e-12)
end
...
# ╔═╡ b8fde8a1-8a32-4861-b438-50da2401977f
md"""
- Como temos $\nabla\phi(x^*) = 0$ e a hessiana $\nabla^2\phi(x^*)$ é definida positiva (autovalores positivos), então $x^*$ é minimizador local.
"""

# ╔═╡ 4ae2a391-dbfc-4d0d-8796-bc3aebb792c8
begin
	x2⁰ = [8,.8]
	df2,  = newton_siseq(∇f, ∇²f, x2⁰,ε = 1e-12)
end
...
# ╔═╡ 44332275-0314-46a8-a5d7-ca7adff9810f
begin
	rosen(x,y) = (1-x)^2 + 100*(y-x^2)^2
	rosen(x) = rosen(x[1],x[2])
	∇rosen(x) = ForwardDiff.gradient(rosen,x)
	∇²rosen(x) = ForwardDiff.hessian(rosen,x)
	x=range(-2, stop=2, length=100)	
	y=range(-3, stop=1.5, length=100)
    l = @layout [a{0.5w} b]
	plot(x, y, rosen,st = [:surface, :contourf], layout = l)
end

# ╔═╡ a871bf4b-7928-404d-bf50-ce3752673d12
begin
	plot(x, y, f,st =  :contourf)
	plot!([Singleton(v) for v ∈ df1.xᵏ], st = [:scatter, :path])
end

# ╔═╡ 07647f8c-28c3-4e99-918a-f6fbe609ec18
begin	
	plot(x, y, f,st =  :contourf)
	plot!([Singleton(v) for v ∈ df2.xᵏ], st = [:scatter, :path])
end

# ╔═╡ 60f74823-ecc4-4d4d-81c3-d7059c26c22a
dfrosen, = newton_siseq(∇rosen,∇²rosen, [-3,-4])
