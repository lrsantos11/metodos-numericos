...
# ╔═╡ 99e765f3-4c34-46b7-865b-81e536e1404f
begin
	using Plots, PlutoUI, ForwardDiff, DataFrames, StatsPlots, LinearAlgebra, LaTeXStrings
	plotly()
end

# ╔═╡ de92c79a-89ad-11ec-3696-f14e3476b727
md"""
##### UFSC/Blumenau
##### MAT1831 - Métodos Numéricos
##### Prof. Luiz-Rafael Santos
###### Semana 10 - Aula 02-03
"""

# ╔═╡ 54622e7f-faca-4c54-b939-a56de0c67fbf
md"""
## Algoritmo Diferença Dividida (Forma de newton)
"""

# ╔═╡ 47ee7731-fbd4-483b-b2f2-f8879526217c
function difdiv(dados_x, dados_y)
	num_dados = length(dados_x)
	n = num_dados - 1
	# Verificar consistencia dos dados
	@assert num_dados == length(dados_y) "Tamanho de dados_x diferente de tamanho de dados_y"
	# Inicializa tabela de dif div.  
	tab = zeros(num_dados,num_dados)
	tab[:,1] = dados_y
	for j ∈ 2:num_dados
		 for i ∈ j:num_dados
		 	tab[i,j] = (tab[i, j-1]  - tab[i-1, j-1]) /(dados_x[i] - dados_x[i-(j-1)])
		 end
	end
...
# ╔═╡ faf6f92b-30a1-4305-8ad9-2b5b3773e8de
tab = difdiv([1,2,4], [1,3,3])

# ╔═╡ 0a22947d-b86f-4c64-be4c-d3acd543875b
function avalia_pol_newton(x̄,
						   dados_x,
						   tab)
	num_dados = length(dados_x)
	# Verificar consistencia dos dados
	@assert (num_dados, num_dados) == size(tab) "Tamanho de dados_x diferente de tamanho da matriz"
	# Coeficientes
	coef = diag(tab)
	val_pol_x̄ = coef[num_dados]
	for j ∈ num_dados-1:-1:1
		val_pol_x̄ = val_pol_x̄*(x̄ - dados_x[j]) + coef[j]
	end
	return val_pol_x̄
end

# ╔═╡ b9c00887-4fb2-43e4-9fb1-b6ff95052b47
x̄ = avalia_pol_newton(3, [1,2,4], tab)

# ╔═╡ 48e3f87e-3025-4e28-8603-183b15a870db
md"""
### Exemplo 
Temperaturas

- Estimar o valor da temperatura no dia $d=13$. 
"""

# ╔═╡ 3037375c-b8a4-4332-98af-874f1ecd7c73
Dia = [3,  6 , 9 , 12 , 15 , 18,  21,  24,  27] 

# ╔═╡ b886a8f5-d81a-4921-b7b7-c7bfb8fad748
...
# ╔═╡ 137abc46-4668-4557-9ec6-5c3d5eca1a4b
begin 
	@df df_T scatter(:Dia, :Temperatura, label = "T/Dia")
	tab₃ = difdiv(df_T.Dia[3:6],df_T.Temperatura[3:6])
	T₃(x) = avalia_pol_newton(x, df_T.Dia[3:6], tab₃)
	plot!(T₃, lw = 2, c = :red, label = "T₃")
	tab₂ = difdiv(df_T.Dia[3:5],df_T.Temperatura[3:5])
	T₂(x) = avalia_pol_newton(x, df_T.Dia[3:5], tab₂)
	plot!(T₂, lw = 2, c = :cyan, label = "T₂")
	tab₁ = difdiv(df_T.Dia[4:5],df_T.Temperatura[4:5])
	T₁(x) = avalia_pol_newton(x, df_T.Dia[4:5], tab₁)
	plot!(T₁, lw = 2, c = :orange, label = "T₁")
	scatter!([T̄,T̄ , T̄], [T₁(T̄),  T₂(T̄), T₃(T̄)], label = "Estimativas")
	tabₙ = difdiv(df_T.Dia,df_T.Temperatura)
	Tₙ(x) = avalia_pol_newton(x, df_T.Dia, tabₙ)
	plot!(Tₙ, lw = 2, c = :pink, label = "Tₙ")
end
